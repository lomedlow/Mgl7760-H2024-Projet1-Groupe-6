...
livre_categorie = db.Table(
    "livre_categorie",
    db.Column("livre_id", db.Integer, db.ForeignKey("livre.id"), primary_key=True),
    db.Column(
        "categorie_id", db.Integer, db.ForeignKey("categorie.id"), primary_key=True
    ),
)
...
class Livre(db.Model):
    ...
    def chercher_par_categorie(cls, categorie_id):
        return (
            cls.query.join(livre_categorie, cls.id == livre_categorie.c.livre_id)
            .join(Categorie, Categorie.id == livre_categorie.c.categorie_id)
            .filter(Categorie.id == categorie_id)
            .all()
        )
    ...
    def chercher_par_auteur(cls, auteur_nom):
        return (
            cls.query.join(livres_auteurs)
            .join(Auteur)
            .filter(Auteur.nom.ilike(f"%{auteur_nom}%"))
            .all()
        )
